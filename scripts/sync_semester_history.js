...
const KEY_PATH = path.join(ROOT, 'scripts', 'serviceAccountKey.json');
const STUDENT_ID = process.argv[2] || '113006277';
...
async function importCatalogFile({ semester, file }) {
  ...
  const parentRef = db.collection('courseCatalogs').doc(semester);
...
async function findUserDoc(studentId) {
  const users = db.collection('ccxpUsers');
  const queries = [
...
async function loadCatalogIndex(semester) {
  const snap = await db.collection('courseCatalogs').doc(semester).collection('courses').get();
